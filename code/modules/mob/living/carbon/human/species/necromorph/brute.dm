...
/mob/living/carbon/human/proc/biobomb(atom/A)
	...
	face_atom(A)
	.= shoot_ability(/datum/extension/shoot/brute_biobomb, A , /obj/item/projectile/bullet/biobomb/weak, accuracy = 50, dispersion = 0, num = 1, windup_time = 1.25 SECONDS, fire_sound = firesound, nomove = 2 SECONDS, cooldown = 12 SECONDS)
	if (.)
...
/datum/extension/shoot/brute_biobomb/fire_animation()
	spawn(4)
		var/mob/living/L = user
		animate(L, transform=L.get_default_transform(),pixel_x = L.default_pixel_x, time = 0.8 SECONDS, flags = ANIMATION_PARALLEL)
